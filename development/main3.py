...
frame_idx = 0
all_metrics = []  # Collect metrics for final evaluation
...
while True:
    ...
    if results.pose_landmarks:
        ...
        # Debug: print metrics every 30 frames
        if frame_idx % 30 == 0:
            print(f"[DEBUG] Frame {frame_idx} metrics: {metrics}")

    # Collect metrics for final evaluation
    all_metrics.append(metrics)

    # ---------------------------
    # Step 5: Overlay metrics & feedback
    # ---------------------------
    annotated_frame = overlay_metrics(annotated_frame, metrics)

    # ---------------------------
    # Step 6: Write annotated video & show debug image
    # ---------------------------
    out.write(annotated_frame)
    cv2.imshow("Pose + Metrics Debug View", annotated_frame)
...
# ---------------------------
# Step 7: Final Shot Evaluation
# ---------------------------

def compute_final_scores(metrics_list):
    """Aggregate metrics and compute scores 1-10 with feedback"""
    elbow_angles = [m['elbow_angle'] for m in metrics_list if m['elbow_angle'] > 0]
    spine_leans = [m['spine_lean'] for m in metrics_list if m['spine_lean'] != 0]
    head_knee_dists = [m['head_knee_dist'] for m in metrics_list if m['head_knee_dist'] != 0]

    def score_range(val, ideal_min, ideal_max):
        if val < ideal_min:
            return max(1, int(10 * val/ideal_min))
        elif val > ideal_max:
            return max(1, int(10 * ideal_max/val))
        else:
            return 10

    scores = {
        "Footwork": {"score": score_range(np.mean(head_knee_dists), 0, 20),
                     "feedback": "Good stride, but front foot could be more aligned"},
        "Head Position": {"score": score_range(np.mean(head_knee_dists), 0, 20),
                          "feedback": "Head slightly forward, keep over front knee"},
        "Swing Control": {"score": score_range(np.mean(elbow_angles), 100, 150),
                          "feedback": "Elbow angle looks okay, maintain smooth swing"},
        "Balance": {"score": score_range(np.mean(spine_leans), -10, 10),
                    "feedback": "Spine lean is stable, good balance"},
        "Follow-through": {"score": score_range(np.mean(elbow_angles), 100, 150),
                           "feedback": "Good follow-through, keep arms extended"}
    }
    return scores
...
print(f"[INFO] Annotated video saved at {output_path}")
print(f"[INFO] Evaluation saved at {eval_path}")
